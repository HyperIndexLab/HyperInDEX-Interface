...
import { PlusIcon, ChevronDownIcon, ChevronLeftIcon } from '@heroicons/react/24/outline';
...
const LiquidityContainer: React.FC<LiquidityContainerProps> = ({ token1 = 'HSK', token2 = 'Select token' }) => {
  ...
  const {
    ...
    handleApprove,
    isApproving
  } = useTokenApproval(token1Data, token2Data, amount1, amount2);

  const { data: allowance1 } = useReadContract({
    address: token1Data?.symbol !== 'HSK' ? token1Data?.address as `0x${string}` : undefined,
    abi: erc20Abi,
  ...
  const { data: allowance2 } = useReadContract({
    address: token2Data?.symbol !== 'HSK' ? token2Data?.address as `0x${string}` : undefined,
    abi: erc20Abi,
    functionName: 'allowance',
    args: userAddress && token2Data ? [userAddress, ROUTER_CONTRACT_ADDRESS] : undefined,
  });
  ...
  const handleAmountChange = (value: string, isToken1: boolean) => {
    if (isFirstProvider) {
      ...
      if (isToken1) {
        setAmount1(value);
        if (amount > 0) {
          const hskPerBlack = Number(poolInfo.reserve0) / Number(poolInfo.reserve1);
        ...
        setAmount2(value);
        if (amount > 0) {
          const blackPerHsk = Number(poolInfo.reserve1) / Number(poolInfo.reserve0);
  ...
  const calculatePoolShare = () => {
    ...
    const share = Number((amount1Big * BigInt(100)) / (totalSupply + amount1Big));
    return share.toFixed(2);
  };

  // 修改 handleSupply 函数
  const handleSupply = async () => {
    if (!token1Data || !token2Data || !amount1 || !amount2 || !userAddress) return;
    ...
    try {
      ...
      if (isToken1HSK || isToken2HSK) {
        ...
        const tokenAddress = isToken1HSK ? token2Data.address : token1Data.address;  // 不需要转换为 WHSK
    ...
    return (
      <div className="bg-base-300/50 rounded-2xl p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm text-base-content/60">Balance: {token?.balance || '0'}</span>
        </div>
  ...
  const renderPriceInfo = () => {
    ...
    if (isFirstProvider) {
      // 首个提供者的情况，使用输入值计算
      price1 = amount1 && amount2 ? (parseFloat(amount2) / parseFloat(amount1)).toFixed(6) : '0';
      price2 = amount1 && amount2 ? (parseFloat(amount1) / parseFloat(amount2)).toFixed(6) : '0';
    } else if (poolInfo) {
      // 使用池子储备计算实际汇率
      // 确保 token1 是 HSK
      ...
      <div className="bg-base-300/50 rounded-2xl p-4 space-y-3">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <div>1 {token1Data?.symbol} = {price1} {token2Data?.symbol}</div>
            <div>1 {token2Data?.symbol} = {price2} {token1Data?.symbol}</div>
          </div>
        </div>
        <div className="flex justify-between items-center">
  ...
  const renderStep2 = () => {
    // 计算价格和份额
    const price1 = poolInfo ? (Number(poolInfo.reserve1) / Number(poolInfo.reserve0)).toFixed(6) : '0';
    const price2 = poolInfo ? (Number(poolInfo.reserve0) / Number(poolInfo.reserve1)).toFixed(6) : '0';
    const poolShare = calculatePoolShare();

    // 检查是否需要授权
    const needsApproval = needApprove.token1 || needApprove.token2;
    const isButtonDisabled = !amount1 || !amount2 || isPending || isWritePending;
    ...
    return (
      <div className="bg-base-200/30 backdrop-blur-sm rounded-3xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-base-200 rounded-3xl p-6">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-base-300">
                <img src={token1Data?.icon_url || ''} className="w-6 h-6 rounded-full" />
          ...
          <div className="bg-base-200 rounded-3xl p-6">
            ...
            <div className="flex justify-between items-center">
              ...
              />
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-base-300">
                <img src={token2Data?.icon_url || ''} className="w-6 h-6 rounded-full" />
        ...
        <button 
          ...
          onClick={() => needsApproval ? handleApprove(needApprove.token1) : handleSupply()}
          disabled={isButtonDisabled}
        >
          {isPending || isWritePending ? 'Processing...' : 
           needApprove.token1 ? `Approve ${token1Data?.symbol}` :
           needApprove.token2 ? `Approve ${token2Data?.symbol}` :
           'Supply'}
        </button>
      </div>
    );
      ...
      >
        {tokenData ? (
          <div className="flex items-center gap-3">
            <img src={tokenData.icon_url || "/img/HSK-LOGO.png"} 
              alt={tokenData.name} 
        ...
        <div className="flex-1">
          {step === 1 ? (
            ...
            <div className="bg-base-200/30 backdrop-blur-sm rounded-2xl p-6">
              ...
              {/* Fee Tier */}
              <div>
                <h3 className="text-xl font-bold text-base-content/60 mb-3">Fee Tier</h3>
                <p className="text-md mb-8">
                  The pool earns 0.3% of all trades proportional to their share of the pool.
                </p>
