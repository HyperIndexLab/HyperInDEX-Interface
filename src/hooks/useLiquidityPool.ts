...
export function useLiquidityPool(token1Data: TokenData | null, token2Data: TokenData | null) {
  const [isFirstProvider, setIsFirstProvider] = useState(false);
  const [poolInfo, setPoolInfo] = useState<PoolInfo | null>(null);
  const [isLoading, setIsLoading] = useState(false);

  // 检查池子是否存在
  const { data: pairAddress, refetch: refetchPairAddress, isLoading: isPairLoading } = useReadContract({
  ...
  // 获取池子信息
  const { data: pairInfo, refetch: refetchPairInfo, isLoading: isReservesLoading } = useReadContract({
    address: pairAddress as `0x${string}`,
    abi: PAIR_ABI,
    functionName: 'getReserves',
  }) as { data: [bigint, bigint, number] | undefined, refetch: () => Promise<any>, isLoading: boolean };

  // 获取 totalSupply
  const { data: totalSupply, refetch: refetchTotalSupply, isLoading: isSupplyLoading } = useReadContract({
    address: pairAddress as `0x${string}`,
    abi: PAIR_ABI,
    functionName: 'totalSupply',
  }) as { data: bigint | undefined, refetch: () => Promise<any>, isLoading: boolean };
  ...
  useEffect(() => {
    if (token1Data && token2Data) {
      ...
      } else if (!isLoading && pairAddress === '0x0000000000000000000000000000000000000000') {
