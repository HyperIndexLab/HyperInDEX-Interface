...
export function useTokenApproval(
  ...
  const [needApprove, setNeedApprove] = useState({ token1: false, token2: false });
  ...
  const { data: allowance1 } = useReadContract({
    ...
    args: userAddress && token1Data ? [userAddress as `0x${string}`, ROUTER_CONTRACT_ADDRESS] : undefined,
  });
  ...
  const { data: allowance2 } = useReadContract({
    ...
    abi: erc20Abi,
    functionName: 'allowance',
    args: userAddress && token2Data ? [userAddress as `0x${string}`, ROUTER_CONTRACT_ADDRESS] : undefined,
  });
  ...
  useEffect(() => {
    ...
    setNeedApprove({
      token1: token1Data?.symbol !== 'HSK' && allowance1 !== undefined && allowance1 < amount1Big,
      token2: token2Data?.symbol !== 'HSK' && allowance2 !== undefined && allowance2 < amount2Big
    });
